...
import type { MapMarker, CorridorLine } from '@/types';
...
// ─── Custom SVG markers ─────────────────────────────────────────────────────
function createMarkerIcon(color: string, size: number, isCritical: boolean): L.DivIcon {
...
function MapLegend() {
  ...
  return (
    ...
    >
      <p className="text-[0.65rem] font-semibold text-white/70 mb-2 uppercase tracking-wider">Risk Level</p>
      {items.map((item) => (
...
export default function MapPanel({ markers, corridors, selectedLga, onSelectLga }: MapPanelProps) {
  const selected = selectedLga ? markers.find((m) => m.name === selectedLga) : null;
    ...
    <div className="relative h-[600px] w-full rounded-2xl overflow-hidden">
      ...
      >
        ...
        {markers.map((m, i) => {
          ...
          const icon = isHq ? HQ_ICON : createMarkerIcon(
              ...
              <Popup>
                ...
                >
                  ...
                  <div style={{ fontSize: '11px', color: '#94a3b8', lineHeight: '1.7' }}>
                    <div>
                      Risk:{' '}
                      <span style={{ color: RISK_COLORS[m.risk_level], fontWeight: 600 }}>
                        {m.risk_level}
                      </span>
                    </div>
                    <div>
        ...
        {corridors.map((c, i) => (
          ...
          >
            <Popup>
              <div style={{ fontFamily: 'Inter, sans-serif', padding: '8px', color: '#e2e8f0', background: 'rgba(15,23,42,0.92)', borderRadius: '8px' }}>
                <strong style={{ fontSize: '12px' }}>{c.name}</strong>
                <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '4px' }}>
                  {c.distance_km} km · Risk:{' '}
                  <span style={{ color: RISK_COLORS[c.risk_level], fontWeight: 600 }}>
                    {c.risk_level}
                  </span>
                </div>
              </div>
