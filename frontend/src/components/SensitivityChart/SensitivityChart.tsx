...
export default function SensitivityChart({ data, top = 10 }: SensitivityChartProps) {
  const chartData = data.slice(0, top);

  return (
    <GlassCard className="p-5 animate-fade-in-up">
      <h3 className="text-sm font-bold text-dark-text mb-4">Feature Sensitivity — Top {top}</h3>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={chartData} layout="vertical">
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
          <XAxis type="number" tick={{ fontSize: 11, fill: '#64748B' }} />
          <YAxis type="category" dataKey="Feature" tick={{ fontSize: 9, fill: '#64748B' }} width={120} />
          <Tooltip
            contentStyle={{
