...
export default function AlertBanner({ alerts, rotateMs = 30_000 }: AlertBannerProps) {
  const [idx, setIdx] = useState(0);

  useEffect(() => {
    if (alerts.length <= 1) return;
    const id = setInterval(() => setIdx((i) => (i + 1) % alerts.length), rotateMs);
  ...
  return (
    <div className="animate-fade-in flex items-center gap-3 rounded-btn border border-un-red/20 bg-un-red/[0.08] px-4 py-2.5 mb-4">
      ...
      <span className="text-[0.82rem] text-surface-400">
        <strong className="text-surface-200">{alert.zone}</strong> — {alert.message}
      </span>
    </div>
  );
}
