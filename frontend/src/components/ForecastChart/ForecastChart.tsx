...
export default function ForecastChart({ data, lga, baseRisk }: ForecastChartProps) {
  const chartData = data.map((p) => ({ name: `M${p.month}`, events: p.predicted_events }));
    ...
    <GlassCard className="p-5 animate-fade-in-up">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={chartData}>
          ...
          />
          <Line type="monotone" dataKey="events" stroke="#009EDB" strokeWidth={2} dot={{ fill: '#009EDB', r: 3 }} activeDot={{ r: 6 }} />
        </LineChart>
      </ResponsiveContainer>
